...
my ($testFile, $gusConfigFile, $sequenceId);
...
while (my ($transcript, $source_id, $gene_na_feature_id, $exon_start, $exon_end, $cds_start, $cds_end, $rev) = $COORDINATE_QUERY_SH->fetchrow_array()) {
  push (@{$coordinates[$counter]}, ($exon_start, $exon_end, $cds_start, $cds_end, $rev));
  $counter++;
}
...
my $oldShift = 0;
$counter = 0;
my ($exon_start, $exon_end, $cds_start, $cds_end, $isRev);
for (my $coordinateFrame;$coordinateFrame<$coordinatesLen;$coordinateFrame++){
    for (my $i=0;$i<5;$i++) {
        if ($i == 0) {
            ($exon_start, $oldShift, $shiftFrame) = &calcCoordinates($shiftFrame, $coordinateFrame, $locationShiftsLen, $oldShift, $i);
        }        
        elsif ($i == 1) {
            ($exon_end, $oldShift, $shiftFrame) = &calcCoordinates($shiftFrame, $coordinateFrame, $locationShiftsLen, $oldShift, $i);
        }
        elsif ($i == 2) {
    ...
    $oldShift = $oldShift;
    $shiftFrame = $shiftFrame;
    push (@{$locationsShifted[$counter]}, ($exon_start, $exon_end, $cds_start, $cds_end, $isRev));
    $counter++;
}
